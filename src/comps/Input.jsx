...
const Inputs = ({
    entering, setEntering, setSubmitting
}) => {
    ...
    // let qs = `/search?location=${entering.town}`;
    // const history = useHistory();
    const [dataFromYelp, setDataFromYelp] = useState("")
    const submittingFx = async (e) => {
        e.preventDefault();
        setSubmitting(entering.town);
        // setFetchedVisible(true);
        if (entering.town !== '') {
            // history.push(qs);
            let dataFromYelp = await fetch('/.netlify/functions/yelp', {
                method: "POST",
                body: JSON.stringify({ location: entering.town })
            }).then(response =>
                response.json()
            )
            console.log(dataFromYelp.businesses)
        }
        // setEntering({
    ...
    return (
        <div className="Inputs">
            ...
            <input
                ...
                name="town"
                // value={entering.town}
                onChange={e => enteringTownFx(e)}
            />

            <button onClick={e => submittingFx(e)}>Search</button>
